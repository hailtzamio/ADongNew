...
class DetailGoodsIssueDocsViewController: BaseViewController {
    var id = 0
    var item:GoodsIssue2? = nil
    ...
    func getData() {
        ...
        showLoading()
        APIClient.getGoodsIssueDoccument(id: id) { result in
            self.stopLoading()
            switch result {
            case .success(let response):
                ...
                if let value = response.data  {
                    ...
                    self.data.append(Information(pKey: "Người nhận",pValue: value.receiver ?? "---"))
                    
                    self.data.append(Information(pKey: "Lý do",pValue: value.reason ?? "---"))
                    
                    self.data.append(Information(pKey: "Xưởng",pValue: value.warehouseName ?? "---"))
    ...
    private func convertData(value:GoodsIssue2) {
        if(value.code == "") {
            value.code = nil
        }
...
extension DetailGoodsIssueDocsViewController: UITableViewDataSource, UITableViewDelegate {
